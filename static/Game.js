...
class Game {
    constructor() {
        ...
        for (let r = 0; r < 8; r++) {
            ...
            for (let c = 0; c < 8; c++) {
                ...
                tile.position.set(pos.x, -4, pos.z)
                this.scene.add(tile);
                tilesRow.push(tile)
            }
    ...
    // isMoveLegal(pawn, dest) {
    //     let posPawn = Game.FindOnGrid(pawn.position.x, pawn.position.z, this.isWhite)
    //     let posDest = Game.FindOnGrid(dest.position.x, dest.position.z, this.isWhite)
    //     let vec = { x: posDest.x - posPawn.x, z: posDest.z - posPawn.z }

    //     let dist = 1;

    //     // check if is taking
    //     if (this.pawnsTable[posDest.x][posDest.z] == 0 && Math.abs(vec.x) == 2 && Math.abs(vec.z) == 2) {
    //         if (this.pawnsTable[posDest.x - (vec.x / 2)][posDest.z - (vec.z / 2)] != (this.isWhite ? 2 : 1)) {
    //             return false
    //         }
    //         this.isTaking = true;
    //         dist = 2;
    ...
    //     // check if tile empty
    //     // check if diagonal (and forward if not promoted)
    //     if (this.pawnsTable[posDest.x][posDest.z] == 0 && (vec.z == -dist || vec.z == dist)) {
    //         if (this.isWhite) {
    //             if (vec.x == -dist || (pawn.isPromoted && vec.x == dist))
    //                 return true;
    //         } else {
    //             if (vec.x == -dist || (pawn.isPromoted && vec.x == dist))
    //                 return true;
    ...
    enemyMove(from, to, isTaking) {
        ...
        if (isTaking) {
            ...
            let loc = Game.ToSceneLoc(take.x, take.z)
            console.log(take, loc)
            let takenPawn = this.pawnsObjTable.find(p => p.position.x == loc.x && p.position.z == loc.z)
            console.log(takenPawn)
            takenPawn.take()
        }
